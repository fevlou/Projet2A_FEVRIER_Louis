...
public class Playground {
    ...
    private Point victoryPoint; // Coordonnées du point de victoire
    public ArrayList<Sprite> getAllSprites() {
        return environment; // Renvoie tous les sprites (solides, pièges, etc.)
    }


    public Playground (String pathName){
        try{//Importation des images et dimensions
            ...
            final Image imageVictory = ImageIO.read(new File("./img/tileSetTest.png"));
            ...
            final int imageVictoryWidth = imageVictory.getWidth(null);
            final int imageVictoryHeight = imageVictory.getHeight(null);
                    ...
                    switch (element){
                        ...
                        case 'O':
                            TrapSprite trap = new TrapSprite(columnNumber * imageTrapWidth,
                                    lineNumber * imageTrapHeight, imageTrap, imageTrapWidth, imageTrapHeight); environment.add(trap);
                            System.out.println("Piège ajouté : " + trap.getBounds());
                            break;
                        case 'V': // Point de victoire
                            victoryPoint = new Point(columnNumber * imageVictoryWidth, lineNumber * imageVictoryHeight);
                            environment.add(new Sprite(columnNumber * imageVictoryWidth,
                                    lineNumber * imageVictoryHeight, imageVictory, imageVictoryWidth, imageVictoryHeight) {
                                @Override
                                public boolean intersect(Rectangle2D.Double hitBox) {
                                    return false;
                                }
    ...
    // Getter pour le point de victoire
    public Point getVictoryPoint() {
        return victoryPoint;
    }
